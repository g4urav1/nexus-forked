    ...
    const userPosts = [
        {
            id: 1,
            time: '2h ago',
            content: 'Just dropped the latest update to Nexus UI! Complete dark-mode overhaul with responsive layouts and smooth micro-interactions. Let me know what you think! 👇',
            likes: 142,
            comments: 18,
            shares: 9,
        ...
        },
        {
            id: 2,
            time: '3 days ago',
            content: 'Quick tip for Tailwind v4: Leverage dynamic CSS variables for effortless dark mode color tokens without cluttering your DOM classes.',
            likes: 89,
            comments: 12,
            shares: 24,
            image: null
                    ...
                    <main className="flex-1 min-w-0 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl sm:rounded-3xl overflow-hidden shadow-sm">
                        ...
                        <div className="px-4 sm:px-6 pb-4 relative">
                            <div className="flex justify-between items-end -mt-12 sm:-mt-16 mb-4">
                                ...
                                {/* Edit Profile Action */}
                                <button className="px-4 py-2 sm:px-5 sm:py-2.5 bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-200 dark:text-slate-900 text-white font-semibold rounded-2xl text-xs sm:text-sm transition shadow-md active:scale-95">
                                    Edit Profile
                                </button>
                            </div>
                            ...
                            <div className="space-y-3">
                                <div>
                                    <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white leading-tight">{user.name}</h1>
                                    <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-medium">{user.handle}</p>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-x-4 gap-y-2 text-xs text-slate-500 dark:text-slate-400 pt-1">
                                    ...
                                    <div className="flex items-center space-x-1">
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m13.35-.622l1.757-1.757a4.5 4.5 0 00-6.364-6.364l-4.5 4.5a4.5 4.5 0 001.242 7.244" /></svg>
                                        <a href={user.website} target="_blank" rel="noreferrer" className="text-indigo-600 dark:text-indigo-400 hover:underline">{user.website.replace('https://', '')}</a>
                                    </div>
                                    <div className="flex items-center space-x-1">
                                ...
                                <div className="flex space-x-5 pt-1 text-xs sm:text-sm">
                                    <div>
                                        <span className="font-bold text-slate-900 dark:text-white">{user.following}</span>{' '}
                                        <span className="text-slate-500 dark:text-slate-400">Following</span>
                                    </div>
                                    <div>
                                        <span className="font-bold text-slate-900 dark:text-white">{user.followers}</span>{' '}
                            ...
                            <div className="flex border-b border-slate-200/80 dark:border-slate-800 mt-6 text-xs sm:text-sm font-semibold">
                                {['posts', 'media', 'likes'].map((tab) => (
                                    <button
                                        ...
                                        className={`py-3 px-4 capitalize relative transition ${activeTab === tab
                        ...
                        <div className="p-4 sm:p-6 space-y-4 bg-slate-50/50 dark:bg-slate-950/30">
                            {activeTab === 'posts' && (
                                userPosts.map((post) => (
                                    <article key={post.id} className="p-4 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl space-y-3 shadow-sm">
                                        <div className="flex items-center space-x-3">
                                            <img src={user.avatar} alt={user.name} className="w-10 h-10 rounded-full object-cover" />
                                            <div>
                                                <h4 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white leading-tight">{user.name}</h4>
                                        ...
                                        <div className="flex items-center space-x-6 text-slate-400 text-xs pt-2">
                                            <button className="flex items-center space-x-1.5 hover:text-indigo-600 dark:hover:text-indigo-400 transition">
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" /></svg>
                                                <span>{post.likes}</span>
                                            </button>
                                            <button className="flex items-center space-x-1.5 hover:text-indigo-600 dark:hover:text-indigo-400 transition">
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.585 1.659l-.48 1.921 2.03-.406c.552-.11 1.115.08 1.542.418A8.93 8.93 0 0012 20.25z" /></svg>
                            ...
                            {activeTab === 'media' && (
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                    {['https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=400&q=80', 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=400&q=80'].map((src, i) => (
                                        <div key={i} className="aspect-square rounded-2xl overflow-hidden bg-slate-800 border border-slate-200/80 dark:border-slate-800">
                                            <img src={src} alt="Media thumbnail" className="w-full h-full object-cover hover:scale-105 transition duration-300" />
                                        </div>
                                    ))}
                                </div>
                            )}
